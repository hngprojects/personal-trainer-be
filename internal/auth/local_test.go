...
func (m *fakeMailer) SendPaidSessionRescheduleConfirmation(_, _ string, _ time.Time, _ int, _, _, _, _, _ string) error {
	return m.err
}
func (m *fakeMailer) SendPaidSessionRescheduleTrainerNotification(_, _, _ string, _ time.Time, _ int, _, _, _, _ string) error {
	return m.err
}
