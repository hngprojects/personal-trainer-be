...
func (f *fakeUserRepo) GetUserByID(_ context.Context, id uuid.UUID) (db.User, error) {
	if f.user != nil {
		return *f.user, f.err
	}
	return db.User{ID: id, Email: "dummy2@example.com", AuthProvider: "local", IsActive: true}, nil
}
