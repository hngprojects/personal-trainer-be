...
SELECT
  id, subscription_id, booking_id, payer_id, provider, provider_transaction_id,
  idempotency_key, currency, total_amount, trainer_earning, platform_fee,
  payment_type, payment_status, paid_at, created_at
FROM payments
...
const flagPaymentForReconciliation = `-- name: FlagPaymentForReconciliation :exec
UPDATE payments
SET payment_status = 'wallet_reconciliation_needed'
WHERE id = $1 AND payment_status = 'successful'
`

func (q *Queries) FlagPaymentForReconciliation(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, flagPaymentForReconciliation, id)
	return err
}
