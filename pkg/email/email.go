...
func (m *SMTPMailer) SendTrainerCredentials(to, password string) error {
	...
	// Quick implementation: reuse the admin HTML template (wording says "admin").
	// Later you can add a dedicated trainer template/subject.
	body, err := adminCredentialsHTML(toAddr, password)
	if err != nil {
	...
	msg := fmt.Sprintf(
		"From: %s\r\nSubject: %s\r\nMIME-Version: 1.0\r\nContent-Type: text/html; charset=UTF-8\r\n\r\n%s",
		fromAddr, "Your trainer account is ready", body,
	)
	return smtp.SendMail(m.host+":"+m.port, auth, fromAddr, []string{toAddr}, []byte(msg))
}
...
func (m *LogMailer) SendTrainerCredentials(to, password string) error {
	body, err := trainerCredentialsHTML(to, password)
	if err != nil {
		return err
	}
	slog.Info("email", "to", to, "subject", trainerCredentialsSubject, "body", body)
	return nil
}
...
const trainerCredentialsSubject = "Your trainer account is ready"
...
<body style="margin:0;padding:0;background:#f4f4f5;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 0;">
	<tr><td align="center">
	  <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;padding:40px;">
		<tr><td align="center" style="padding-bottom:16px;">
		  <h2 style="margin:0;font-size:22px;color:#111827;">Your trainer account is ready</h2>
		</td></tr>
		<tr><td align="center" style="padding-bottom:24px;">
		  <p style="margin:0;font-size:14px;color:#6b7280;">A trainer account was created for you. Use the credentials below to sign in.</p>
		</td></tr>
		<tr><td style="padding:8px 0;">
		  <p style="margin:0;font-size:14px;color:#111827;"><strong>Email:</strong> {{ .Email }}</p>
		</td></tr>
